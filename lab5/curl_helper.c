...
int find_http(char *buf, int size, int follow_relative_links, const char *base_url, url_node ** frontier_stack, Hashtable ** all_visited_urls)
{
...
int process_html(CURL *curl_handle, RECV_BUF *p_recv_buf, url_node ** stack, Hashtable ** all_visited_urls)
{
    int follow_relative_link = 1;
    char *url = NULL; 

    curl_easy_getinfo(curl_handle, CURLINFO_EFFECTIVE_URL, &url);
    find_http(p_recv_buf->buf, p_recv_buf->size, follow_relative_link, url, stack, all_visited_urls);
...
int process_data(CURL *curl_handle, RECV_BUF *p_recv_buf, html_struct * html_args, url_node** url_frontier, Hashtable** pngTable, Hashtable ** all_visited_urls, char* url)
{
    ...
    if ( strstr(ct, CT_HTML) ) {
        result = process_html(curl_handle, p_recv_buf, url_frontier, all_visited_urls);
        if (result != 0) {
            return -3;
        }
    } else if ( strstr(ct, CT_PNG) ) {
        isPng = process_png(curl_handle, p_recv_buf, pngTable);
        if (isPng < 0) {
            return -3;
        }
    } else {}
